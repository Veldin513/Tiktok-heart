...
class DiagnosticsApp(ttk.Frame):
    ...
    def __init__(
        ...
        self.worker_schedule_status_var = tk.StringVar(value='Автозапуск: проверка…')
        self.worker_schedule_at_logon_var = tk.BooleanVar(value=True)
        self.worker_schedule_every_12h_var = tk.BooleanVar(value=True)
        ...
        self.refresh_worker_schedule_status(update_status=False)
    ...
    def _build_menu(self) -> None:
        ...
        actions_menu.add_command(label='Центр настройки…', command=self.show_setup_center_dialog)
        ...
        actions_menu.add_command(label='Автозапуск worker…', command=self.show_worker_schedule_dialog)
        actions_menu.add_command(label='Импорт профиля из Google Chrome…', command=self.show_google_profile_import_dialog)
        actions_menu.add_separator()
    ...
    def _build_header(self) -> None:
        hdr = tk.Frame(self, bg=C['panel'], height=76)
        hdr.pack(fill=tk.X, padx=0, pady=0)
        hdr.pack_propagate(False)

        tk.Frame(hdr, bg=C['accent2'], height=2).place(relx=0, rely=1.0, relwidth=1, anchor='sw')

        title_box = tk.Frame(hdr, bg=C['panel'])
        title_box.pack(side=tk.LEFT, fill=tk.BOTH, expand=True, padx=18, pady=9)
        tk.Label(title_box, text='TikTok Heart Bot',
                 bg=C['panel'], fg=C['text_head'], font=('Segoe UI', 19, 'bold')).pack(anchor=tk.W)
        tk.Label(title_box, textvariable=self.hero_subtitle_var,
                 bg=C['panel'], fg='#cbd5e1', font=FONT_SMALL).pack(anchor=tk.W, pady=(2, 0))
        ...
        build_badge.pack(anchor=tk.W, pady=(2, 0))

        chip_box = tk.Frame(hdr, bg=C['panel'])
        chip_box.pack(side=tk.RIGHT, padx=18, pady=12)
    ...
    def _chip(self, parent: tk.Widget, text: str) -> tk.Label:
        return tk.Label(parent, text=text, bg=C['chip_idle'], fg='white',
                        padx=10, pady=4, font=('Segoe UI', 8, 'bold'))
    ...
    def _build_quick_bar(self) -> None:
        ...
        row = tk.Frame(bar, bg=C['surface2'])
        row.pack(fill=tk.X, padx=14, pady=(7, 7))

        left = tk.Frame(row, bg=C['surface2'])
        left.pack(side=tk.LEFT, fill=tk.X, expand=True)
        actions = tk.Frame(left, bg=C['surface2'])
        actions.pack(anchor='w')
        self._qbtn(actions, 'Центр настройки', self.show_setup_center_dialog, C['accent2']).pack(side=tk.LEFT, padx=(0, 6))
        self._qbtn(actions, 'Запустить', self.start_all, C['ok']).pack(side=tk.LEFT, padx=(0, 6))
        self._qbtn(actions, 'Стоп', self.stop_all, C['danger']).pack(side=tk.LEFT, padx=(0, 6))
        self._qbtn(actions, 'Обновить', self.refresh_all, '#475569').pack(side=tk.LEFT, padx=(0, 8))
        self.mode_toggle_btn = self._qbtn(actions, 'LIVE', self.toggle_dry_run_mode, C['accent'])
        self.mode_toggle_btn.pack(side=tk.LEFT, padx=(0, 8))
        self._menu_btn(actions, 'Сервисы', [
            ('Worker: запустить', self.start_worker),
            ('Worker: остановить', self.stop_worker),
            ('Worker: перезапустить', self.restart_worker),
            None,
            ('Telegram: запустить', self.start_telegram_bot),
            ('Telegram: остановить', self.stop_telegram_bot),
            ('Telegram: перезапустить', self.restart_telegram_bot),
            ('Telegram: очистить lock', self.clear_telegram_lock),
            None,
            ('Пауза worker', self.toggle_pause),
            ('Сбросить runtime-флаги', self.reset_runtime_flags),
        ]).pack(side=tk.LEFT, padx=(0, 6))
        self._menu_btn(actions, 'Профиль', [
            ('Импорт из Google Chrome…', self.show_google_profile_import_dialog),
            ('Автозапуск worker…', self.show_worker_schedule_dialog),
            None,
            ('Открыть profiles', lambda: self.open_path(self.adapter.base_dir / 'profiles')),
            ('Логи профиля', lambda: self.open_path(self.adapter.get_active_profile_logs_dir())),
            ('Backups', self.open_backups_folder),
        ]).pack(side=tk.LEFT, padx=(0, 6))
        self._menu_btn(actions, 'Ещё', [
            ('Копировать текущий раздел', self.copy_current_tab),
        ...
        options = tk.Frame(right, bg=C['surface2'])
        ...
        self.worker_schedule_button = tk.Button(
            options,
            textvariable=self.worker_schedule_status_var,
            command=self.show_worker_schedule_dialog,
            bg=C['surface2'],
            fg=C['accent'],
            activebackground=C['accent_bg'],
            activeforeground=C['accent'],
        ...
        self.worker_schedule_button.pack(side=tk.LEFT)
    ...
    def _qbtn(self, parent, text: str, cmd, color: str) -> tk.Button:
        btn = tk.Button(parent, text=text, command=cmd,
                        ...
                        font=FONT_BOLD, padx=11, pady=5, cursor='hand2',
    ...
    def _build_tabs(self) -> None:
        ...
        self.tabs.add(self.services_tab,    text='  Сервисы  ')
    ...
    def _build_overview_tab(self) -> None:
        ...
        self.metric_cards = {
            'worker':   self._metric_card(cards_col, 0, 0, 'Worker', height=92),
            ...
            'messages': self._metric_card(cards_col, 1, 1, 'Сообщения', height=92),
        }
    ...
    def _metric_card(self, parent, row, col, title, icon='', *, height: int = 132):
        ...
        tk.Label(hdr, text=title, bg=C['surface'], fg=C['text_sub'],
                 font=FONT_BOLD).pack(side=tk.LEFT, padx=(5 if icon else 0, 0))
    ...
    def _build_profiles_tab(self) -> None:
        ...
        ttk.Button(ctrl, text='Импорт из Chrome',
                   command=self.show_google_profile_import_dialog).pack(side=tk.LEFT, padx=(0, 6))
    ...
    def _health_state(self, score: int, status: str | None = None, label: str | None = None) -> tuple[str, str]:
        status = str(status or '').lower()
        if status == 'critical':
            return label or 'Есть блокер', 'danger'
        if status == 'warning':
            return label or 'Нужно внимание', 'warning'
        if status == 'ok':
            return label or 'Готово', 'ok'
        if score >= 85:
            return 'Стабильно', 'ok'
        if score >= 65:
    ...
    def _update_header(self, diag: dict[str, object]) -> None:
        ...
        health_lbl, health_kind = self._health_state(
            score, str(health.get('status') or ''), str(health.get('label') or ''))
    ...
    def _fill_overview(self, diag: dict[str, object]) -> None:
        ...
        self._draw_health_gauge(
            score,
            str(health.get('summary') or '—'),
            str(health.get('status') or ''),
            str(health.get('label') or ''),
        )
        ...
        self._update_metric_card(
            ...
            f"Профилей: {profiles.get('total', 0)}   Browser: {'OK' if browser.get('default_profile_exists') else 'нужен вход/импорт'}",
        ...
        rows = [
            ...
            ('Браузер', 'OK' if browser.get('default_profile_exists') else 'Нужен вход',
             f"user_data: {'да' if browser.get('exists') else 'нет'}  ·  cookies: {'да' if browser.get('cookies_exists') else 'нет'}  ·  backoff: {self._format_duration_short(browser.get('auth_backoff_left')) if browser.get('auth_backoff_left') else '—'}"),
            ('Автозапуск', 'Вкл' if schedule.get('installed') else 'Выкл',
             f"вход: {'да' if schedule.get('at_logon') else 'нет'}  ·  12ч: {'да' if schedule.get('every_12_hours') else 'нет'}  ·  след.: {schedule.get('next_run_time') or '—'}"),
        ...
        summary_lines = [
            ...
            f'Браузер          : {"готов" if browser.get("default_profile_exists") else "нужен вход/импорт"}',
            f'Автозапуск       : {"включён" if schedule.get("installed") else "выключен"}',
    ...
    def _draw_health_gauge(
        ...
        label, kind = self._health_state(score, status, label_override)
        color = C['ok'] if kind == 'ok' else C['warning'] if kind == 'warning' else C['danger']
    ...
    def _fill_diagnostics(self, diag: dict[str, object]) -> None:
        ...
        browser = dict(diag.get('browser_profile') or {})
        chrome = dict(diag.get('chrome_profiles') or {})
        schedule = dict(diag.get('worker_schedule') or {})
        ...
        lines.extend([
            ...
            '',
            '── Браузер / авторизация ──',
            f"Chromium user_data : {'есть' if browser.get('exists') else 'нет'}",
            f"Default profile    : {'есть' if browser.get('default_profile_exists') else 'нет'}",
            f"Cookies            : {'есть' if browser.get('cookies_exists') else 'нет'}",
            f"Auth backoff       : {self._format_duration_short(browser.get('auth_backoff_left')) if browser.get('auth_backoff_left') else 'нет'}",
            f"Chrome profiles    : {chrome.get('total', 0)}",
            '',
            '── Автозапуск worker ──',
            f"Задача       : {'включена' if schedule.get('installed') else 'выключена'}",
            f"При входе    : {'да' if schedule.get('at_logon') else 'нет'}",
            f"Каждые 12 ч  : {'да' if schedule.get('every_12_hours') else 'нет'}",
            f"Следующий    : {schedule.get('next_run_time') or '—'}",
            '',
        ])
    ...
    def _fill_files(self, diag: dict[str, object]) -> None:
        ...
        browser = dict(diag.get('browser_profile') or {})
        for kind, path, ok, note in [
            ('browser_profile', browser.get('user_data_dir'), browser.get('exists'), 'Основной Chromium user_data для worker.'),
            ('browser_default', str(Path(str(browser.get('user_data_dir') or '')) / 'Default') if browser.get('user_data_dir') else '', browser.get('default_profile_exists'), 'Папка Default внутри Chromium-профиля.'),
            ('browser_cookies', browser.get('cookies_path'), browser.get('cookies_exists'), 'Файл cookies помогает понять, перенеслась ли сессия браузера.'),
            ('auth_backoff', browser.get('auth_backoff_path'), browser.get('auth_backoff_exists'), 'Локальный флаг ожидания после ошибки TikTok “too many attempts”.'),
        ]:
            if not path:
                continue
            path_text = str(path)
            status = 'OK' if ok else '⚠ Не найден'
            self.files_tree.insert('', tk.END, values=(kind, status, '—', '—', path_text))
            self._file_notes_by_path[path_text] = (
                f"Тип   : {kind}\n"
                f"Статус: {status}\n"
                f"Путь  : {path_text}\n\n"
                f"{note}")
    ...
    def show_google_profile_import_dialog(self) -> None:
        try:
            chrome_profiles = self.adapter.list_google_chrome_profiles()
        except Exception as exc:
            messagebox.showerror('Импорт из Google Chrome', f'Не удалось прочитать профили Chrome:\n{exc}')
            return
        if not chrome_profiles:
            messagebox.showwarning(
                'Импорт из Google Chrome',
                'Профили Google Chrome не найдены. Сначала войди в TikTok в обычном Chrome.',
            )
            return

        bot_profiles = self.adapter.get_profiles()
        if not bot_profiles:
            messagebox.showwarning('Импорт из Google Chrome', 'В приложении нет профилей бота для импорта.')
            return

        dlg = tk.Toplevel(self.master)
        dlg.title('Импорт профиля из Google Chrome')
        ...
        tk.Label(body, text='Импорт профиля из Google Chrome',
                 bg=C['surface'], fg=C['text_main'], font=FONT_TITLE).pack(anchor='w')
        tk.Label(
            body,
            text='По умолчанию копируются только TikTok cookies и небольшой session/storage набор. Старый user_data сохранится в backup.',
            bg=C['surface'], fg=C['text_sub'], font=FONT_SMALL, wraplength=520, justify=tk.LEFT,
        ...
        chrome_by_choice = dict(zip(chrome_choices, chrome_profiles))
        chrome_var = tk.StringVar(value=chrome_choices[0])
        for choice, item in chrome_by_choice.items():
            if item.get('id') == 'Default':
                chrome_var.set(choice)
                break

        bot_choices = [f"{profile.label}  [{profile.key}]" for profile in bot_profiles]
        bot_by_choice = dict(zip(bot_choices, bot_profiles))
        selected_idx = self._selected_profile_index()
        default_bot_idx = 0
        if selected_idx is not None and 0 <= selected_idx < len(bot_profiles):
            default_bot_idx = selected_idx
        else:
            for idx, profile in enumerate(bot_profiles):
                if profile.active:
                    default_bot_idx = idx
                    break
        bot_var = tk.StringVar(value=bot_choices[default_bot_idx])
        mode_choices = {
            'Только TikTok session (рекомендуется)': 'tiktok_session',
            'Полный профиль Chrome': 'full_profile',
        }
        mode_var = tk.StringVar(value=next(iter(mode_choices)))

        form = tk.Frame(body, bg=C['surface'])
        form.pack(fill=tk.X)
        tk.Label(form, text='Профиль Chrome', bg=C['surface'], fg=C['text_sub'], font=FONT_BOLD).grid(row=0, column=0, sticky='w', pady=(0, 6))
        chrome_combo = ttk.Combobox(form, textvariable=chrome_var, values=chrome_choices, state='readonly', width=52)
        chrome_combo.grid(row=1, column=0, sticky='ew', pady=(0, 12))
        tk.Label(form, text='Профиль бота', bg=C['surface'], fg=C['text_sub'], font=FONT_BOLD).grid(row=2, column=0, sticky='w', pady=(0, 6))
        bot_combo = ttk.Combobox(form, textvariable=bot_var, values=bot_choices, state='readonly', width=52)
        bot_combo.grid(row=3, column=0, sticky='ew', pady=(0, 12))
        tk.Label(form, text='Режим импорта', bg=C['surface'], fg=C['text_sub'], font=FONT_BOLD).grid(row=4, column=0, sticky='w', pady=(0, 6))
        mode_combo = ttk.Combobox(form, textvariable=mode_var, values=list(mode_choices), state='readonly', width=52)
        mode_combo.grid(row=5, column=0, sticky='ew')
        form.columnconfigure(0, weight=1)

        tk.Label(
            body,
            text='Перед импортом закрой все окна Google Chrome и останови worker.',
            bg=C['surface'], fg=C['warning'], font=FONT_SMALL, wraplength=520, justify=tk.LEFT,
        ).pack(anchor='w', pady=(12, 14))

        footer = tk.Frame(body, bg=C['surface'])
        footer.pack(fill=tk.X)

        result: dict[str, object] = {}

        def apply_import() -> None:
            chrome_profile = chrome_by_choice.get(chrome_var.get())
            bot_profile = bot_by_choice.get(bot_var.get())
            copy_mode = mode_choices.get(mode_var.get(), 'tiktok_session')
            if not chrome_profile or not bot_profile:
                messagebox.showwarning('Импорт из Google Chrome', 'Выбери профиль Chrome и профиль бота.')
                return
            if not messagebox.askyesno(
                'Импорт из Google Chrome',
                f'Импортировать «{chrome_profile.get("label") or chrome_profile.get("id")}» '
                f'в профиль бота «{bot_profile.label}»?\n\n'
                f'Режим: {mode_var.get()}.\n'
                'Текущий Chromium user_data будет перенесён в backup только после успешной подготовки нового профиля.',
            ):
                return

            def action() -> None:
                result.clear()
                result.update(self.adapter.import_google_chrome_profile(
                    chrome_profile_id=str(chrome_profile.get('id') or 'Default'),
                    bot_profile_key=bot_profile.key,
                    copy_mode=copy_mode,
                ))

            def success_message() -> str:
                backup = str(result.get('backup') or '')
                backup_note = f' Backup: {Path(backup).name}' if backup else ''
                mode = 'TikTok session' if result.get('copy_mode') == 'tiktok_session' else 'полный профиль'
                size_note = f" Размер: {self._format_file_size(int(result.get('size_bytes') or 0))}."
                cookies_note = ''
                cookies = result.get('cookies')
                if isinstance(cookies, dict) and cookies.get('copied'):
                    cookies_note = f" Cookies: {cookies.get('kept', 0)}/{cookies.get('before', 0)}."
                return (
                    f'Профиль Chrome импортирован в «{result.get("bot_profile_key") or bot_profile.key}». '
                    f'Режим: {mode}.{size_note}{cookies_note}{backup_note}'
                )

            self._run_action(
                action,
                success_message,
                'Ошибка импорта профиля из Google Chrome',
                on_success=lambda: self.refresh_all(),
            )
            dlg.destroy()

        ttk.Button(footer, text='Отмена', command=dlg.destroy).pack(side=tk.RIGHT)
        ttk.Button(footer, text='Импортировать', command=apply_import).pack(side=tk.RIGHT, padx=(0, 8))

        dlg.update_idletasks()
        x = self.master.winfo_rootx() + max(40, (self.master.winfo_width() - dlg.winfo_width()) // 2)
        y = self.master.winfo_rooty() + max(40, (self.master.winfo_height() - dlg.winfo_height()) // 3)
        dlg.geometry(f'+{x}+{y}')
        ...
        *on_success* is an optional zero-argument callable invoked on the main
        ...
        def worker() -> None:
            try:
                ...
                error_message = f'{error_title}: {exc}'
                self.master.after(
                    0, lambda: self._finish_action(error_message, is_error=True))
                return
    ...
    # Service actions
    # ══════════════════════════════════════════════════════════════════════════
    def _worker_schedule_trigger_text(self, at_logon: bool | None = None, every_12_hours: bool | None = None) -> str:
        at_logon = self.worker_schedule_at_logon_var.get() if at_logon is None else at_logon
        every_12_hours = self.worker_schedule_every_12h_var.get() if every_12_hours is None else every_12_hours
        parts = []
        if at_logon:
            parts.append('вход')
        if every_12_hours:
            parts.append('12 ч')
        return ' + '.join(parts) if parts else 'триггеры не выбраны'

    def refresh_worker_schedule_status(self, *, update_status: bool = True) -> None:
        def worker() -> None:
            status = self.adapter.get_worker_schedule_status()
            self.master.after(0, lambda s=status: self._apply_worker_schedule_status(s, update_status=update_status))

        threading.Thread(target=worker, daemon=True, name='schedule-status').start()

    def _apply_worker_schedule_status(self, status: dict[str, object], *, update_status: bool) -> None:
        if not isinstance(status, dict):
            self.worker_schedule_status_var.set('Автозапуск: неизвестно')
            return
        error = str(status.get('error') or '').strip()
        if error:
            self.worker_schedule_status_var.set('Автозапуск: ошибка')
            if update_status:
                self.set_status(f'Ошибка проверки автозапуска: {error}')
            return
        if not bool(status.get('available', True)):
            self.worker_schedule_status_var.set('Автозапуск: недоступен')
            return
        if not bool(status.get('installed')):
            self.worker_schedule_status_var.set('Автозапуск: выключен')
            self.worker_schedule_at_logon_var.set(True)
            self.worker_schedule_every_12h_var.set(True)
            if update_status:
                self.set_status('Автозапуск worker выключен')
            return

        at_logon = bool(status.get('at_logon'))
        every_12_hours = bool(status.get('every_12_hours'))
        self.worker_schedule_at_logon_var.set(at_logon)
        self.worker_schedule_every_12h_var.set(every_12_hours)
        text = self._worker_schedule_trigger_text(at_logon, every_12_hours)
        state = str(status.get('state') or '').strip()
        suffix = f' ({state})' if state and state.lower() not in {'ready', 'running'} else ''
        self.worker_schedule_status_var.set(f'Автозапуск: {text}{suffix}')
        if update_status:
            self.set_status(f'Автозапуск worker: {text}{suffix}')

    def show_worker_schedule_dialog(self) -> None:
        self.refresh_worker_schedule_status(update_status=False)
        dlg = tk.Toplevel(self.master)
        dlg.title('Автозапуск worker')
        dlg.transient(self.master)
        dlg.configure(bg=C['bg'])
        dlg.resizable(False, False)
        ...
        tk.Label(body, text='Автозапуск worker', bg=C['surface'],
                 fg=C['text_main'], font=FONT_TITLE).pack(anchor='w')
        tk.Label(body, text='Это задача Планировщика Windows, не отдельная служба.',
                 bg=C['surface'], fg=C['text_sub'], font=FONT_SMALL).pack(anchor='w', pady=(4, 12))

        ttk.Checkbutton(body, text='Запускать при входе в Windows',
                        variable=self.worker_schedule_at_logon_var).pack(anchor='w', pady=(0, 8))
        ttk.Checkbutton(body, text='Запускать каждые 12 часов',
                        variable=self.worker_schedule_every_12h_var).pack(anchor='w', pady=(0, 10))

        tk.Label(body, textvariable=self.worker_schedule_status_var,
                 bg=C['surface'], fg=C['accent'], font=FONT_BOLD).pack(anchor='w', pady=(2, 14))

        footer = tk.Frame(body, bg=C['surface'])
        footer.pack(fill=tk.X)

        def apply_and_close() -> None:
            if not self.worker_schedule_at_logon_var.get() and not self.worker_schedule_every_12h_var.get():
                messagebox.showwarning('Триггеры не выбраны', 'Выбери хотя бы один триггер автозапуска.')
                return
            self.apply_worker_schedule()
            dlg.destroy()

        def disable_and_close() -> None:
            self.disable_worker_schedule()
            dlg.destroy()

        ttk.Button(footer, text='Отключить', command=disable_and_close).pack(side=tk.LEFT)
        ttk.Button(footer, text='Закрыть', command=dlg.destroy).pack(side=tk.RIGHT)
        ttk.Button(footer, text='Применить', command=apply_and_close).pack(side=tk.RIGHT, padx=(0, 8))
    ...
    def apply_worker_schedule(self) -> None:
        if not self.worker_schedule_at_logon_var.get() and not self.worker_schedule_every_12h_var.get():
            messagebox.showwarning('Триггеры не выбраны', 'Выбери хотя бы один триггер автозапуска.')
            return
        at_logon = bool(self.worker_schedule_at_logon_var.get())
        every_12_hours = bool(self.worker_schedule_every_12h_var.get())
        self._run_action(
            lambda: self.adapter.register_worker_schedule(at_logon=at_logon, every_12_hours=every_12_hours),
            lambda: f'Автозапуск worker включён: {self._worker_schedule_trigger_text(at_logon, every_12_hours)}',
            'Ошибка настройки автозапуска worker',
            on_success=lambda: self.refresh_worker_schedule_status(update_status=False),
        )

    def disable_worker_schedule(self) -> None:
        self._run_action(
            self.adapter.unregister_worker_schedule,
            'Автозапуск worker выключен',
            'Ошибка отключения автозапуска worker',
            on_success=lambda: self.refresh_worker_schedule_status(update_status=False),
        )

    def _format_setup_center_report(self, diag: dict[str, object]) -> str:
        worker = dict(diag.get('worker') or {})
        telegram = dict(diag.get('telegram_bot') or {})
        state = dict(diag.get('state') or {})
        health = dict(diag.get('health') or {})
        ...
        schedule = dict(diag.get('worker_schedule') or {})
        chrome = dict(diag.get('chrome_profiles') or {})
        msg = dict(diag.get('message_pool_details') or {})
        profiles = dict(diag.get('profiles') or {})

        def yes(value: object) -> str:
            return 'да' if bool(value) else 'нет'

        backoff_left = int(browser.get('auth_backoff_left') or 0)
        backoff_text = 'нет'
        if backoff_left > 0:
            backoff_text = f'{backoff_left // 3600}ч {(backoff_left % 3600) // 60}м'

        schedule_bits = []
        if schedule.get('at_logon'):
            schedule_bits.append('вход в Windows')
        if schedule.get('every_12_hours'):
            schedule_bits.append('каждые 12 ч')
        schedule_text = ' + '.join(schedule_bits) if schedule_bits else 'выключен'
        if schedule.get('error'):
            schedule_text = 'ошибка проверки'

        latest_backup = ''
        backups = list(browser.get('latest_backups') or [])
        if backups:
            latest_backup = str(dict(backups[0]).get('name') or '')

        lines = [
            'Центр настройки',
            '',
            f"Здоровье           : {health.get('score', 0)}/100 — {health.get('summary') or '—'}",
            f"Активный профиль  : {state.get('active_profile') or '—'}",
            f"Worker            : {'запущен' if worker.get('running') else 'остановлен'}  PID: {worker.get('pid') or '—'}",
            f"Telegram          : {'запущен' if telegram.get('running') else 'остановлен'}  PID: {telegram.get('pid') or '—'}",
            f"Сообщения         : {msg.get('unique_count', 0)} уникальных, дублей: {msg.get('duplicates', 0)}",
            f"Профили бота      : {profiles.get('total', 0)} всего",
            '',
            'Браузер и авторизация',
            f"Chromium user_data : {yes(browser.get('exists'))}",
            f"Default profile    : {yes(browser.get('default_profile_exists'))}",
            f"Local State        : {yes(browser.get('local_state_exists'))}",
            f"Preferences        : {yes(browser.get('preferences_exists'))}",
            f"Cookies            : {yes(browser.get('cookies_exists'))}",
            f"DPAPI recovery     : {'нужен' if browser.get('needs_recovery') else 'не нужен'}",
            f"Auth backoff       : {backoff_text}",
            f"Backups            : {browser.get('backup_count', 0)}" + (f"  последний: {latest_backup}" if latest_backup else ''),
            f"Профили Chrome     : {chrome.get('total', 0)}",
            '',
            'Автозапуск',
            f"Задача             : {'включена' if schedule.get('installed') else 'выключена'}",
            f"Триггеры           : {schedule_text}",
            f"Следующий запуск   : {schedule.get('next_run_time') or '—'}",
            f"Последний результат: {schedule.get('last_task_result') if schedule.get('last_task_result') is not None else '—'}",
            '',
            'Что стоит сделать',
        ]
        ...
        if recommendations:
            ...
            lines.append('• Критичных действий сейчас нет.')

        if chrome.get('error'):
            lines.extend(['', f"Chrome profile error: {chrome.get('error')}"])
        if schedule.get('error'):
            lines.extend(['', f"Schedule error: {schedule.get('error')}"])
        return '\n'.join(lines)

    def show_setup_center_dialog(self) -> None:
        dlg = tk.Toplevel(self.master)
        dlg.title('Центр настройки')
        dlg.transient(self.master)
        dlg.geometry('860x640')
        dlg.minsize(760, 520)
        dlg.configure(bg=C['bg'])

        body = tk.Frame(dlg, bg=C['surface'], padx=14, pady=12)
        body.pack(fill=tk.BOTH, expand=True)
        header = tk.Frame(body, bg=C['surface'])
        header.pack(fill=tk.X, pady=(0, 10))
        tk.Label(header, text='Центр настройки', bg=C['surface'],
                 fg=C['text_main'], font=FONT_TITLE).pack(side=tk.LEFT)
        tk.Button(header, text='Обновить', command=lambda: refresh(),
        ...
        report = UnifiedScrolledText(
            body, wrap=tk.WORD, font=FONT_MONO, relief=tk.FLAT,
            bg=C['surface'], bd=0, highlightthickness=1,
            highlightbackground=C['border'])
        ...
        def action_button(text: str, command, color: str = '#475569') -> None:
            tk.Button(actions, text=text, command=command,
                      bg=color, fg='white', activebackground=color,
                      relief='flat', font=FONT_BOLD, padx=10, pady=6,
                      cursor='hand2', bd=0, highlightthickness=0).pack(side=tk.LEFT, padx=(0, 6))

        def open_browser_profile() -> None:
            browser = dict(last_diag.get('browser_profile') or {})
            path = browser.get('user_data_dir') or self.adapter.browser_profile_summary().get('user_data_dir')
            self.open_path(Path(str(path)))

        def refresh() -> None:
            report.configure(state=tk.NORMAL)
            report.delete('1.0', tk.END)
            report.insert(tk.END, 'Обновляю состояние…')
            ...
            def worker() -> None:
                try:
                    ...
                    self.master.after(0, lambda e=exc: apply_error(e))
                    return
                self.master.after(0, lambda d=diag: apply_diag(d))

            threading.Thread(target=worker, daemon=True, name='setup-center-refresh').start()

        def apply_error(exc: Exception) -> None:
            report.configure(state=tk.NORMAL)
            report.delete('1.0', tk.END)
            report.insert(tk.END, f'Ошибка обновления центра настройки:\n{exc}')
            report.configure(state=tk.DISABLED)

        def apply_diag(diag: dict[str, object]) -> None:
            last_diag.clear()
            last_diag.update(diag)
            report.configure(state=tk.NORMAL)
            report.delete('1.0', tk.END)
            report.insert(tk.END, self._format_setup_center_report(diag))
            report.configure(state=tk.DISABLED)

        action_button('Импорт из Chrome', self.show_google_profile_import_dialog, C['accent2'])
        action_button('Автозапуск', self.show_worker_schedule_dialog, C['accent'])
        action_button('Запустить worker', self.start_worker, C['ok'])
        action_button('Сбросить флаги', self.reset_runtime_flags, '#64748b')
        action_button('Открыть профиль', open_browser_profile, '#475569')
        action_button('Логи', lambda: self.open_path(self.adapter.get_active_profile_logs_dir()), '#475569')
        tk.Button(actions, text='Закрыть', command=dlg.destroy,
                  bg=C['surface2'], fg=C['text_main'], activebackground=C['accent_bg'],
                  relief='flat', font=FONT_BOLD, padx=10, pady=6,
