...
from yara_app.config import BASE_DIR
...
def _chrome_dpapi_key_readable(local_state_path: Path) -> bool | None:
    try:
        payload = json.loads(local_state_path.read_text(encoding='utf-8'))
        encrypted_key_text = payload.get('os_crypt', {}).get('encrypted_key')
        if not encrypted_key_text:
            return None
        encrypted_key = base64.b64decode(encrypted_key_text)
    except Exception:
        return None
    ...
    if not encrypted_key.startswith(prefix):
        return None
    return _windows_dpapi_unprotects(encrypted_key[len(prefix):])


def _windows_dpapi_unprotects(encrypted_payload: bytes) -> bool | None:
    if os.name != 'nt':
        return None
    try:
        import ctypes
        ...
        input_buffer = ctypes.create_string_buffer(encrypted_payload)
        input_blob = DataBlob(
...
def browser_profile_needs_recovery(user_data_dir: Path) -> bool:
    if os.name != 'nt':
        return False
    local_state_path = user_data_dir / 'Local State'
    if not local_state_path.exists():
        return False
    return _chrome_dpapi_key_readable(local_state_path) is False


def recover_browser_profile_after_reinstall(paths: ProfilePaths) -> Path | None:
    if not browser_profile_needs_recovery(paths.user_data_dir):
        return None

    timestamp = time.strftime('%Y%m%d_%H%M%S')
    backup_dir = paths.browser_dir / f'user_data_backup_{timestamp}'
    suffix = 1
    while backup_dir.exists():
        suffix += 1
        backup_dir = paths.browser_dir / f'user_data_backup_{timestamp}_{suffix}'

    shutil.move(str(paths.user_data_dir), str(backup_dir))
...
def init_auth_runtime() -> dict:
    return {
        ...
        'auth_rate_limit_notified': False,
        'auth_rate_limit_until': None,
    }
