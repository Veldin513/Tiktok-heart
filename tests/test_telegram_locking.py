...
from yara_app.telegram_control_bot import TelegramControlBot
...
def test_start_telegram_bot_reuses_live_lock_pid(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr('yara_app.project_adapter.subprocess.Popen', fake_popen)
...
def test_telegram_bot_stops_on_http_409_conflict(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr('yara_app.telegram_control_bot.atexit.register', lambda fn: None)
    monkeypatch.setattr('yara_app.telegram_control_bot.time.sleep', lambda seconds: None)
